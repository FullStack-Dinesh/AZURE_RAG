...
print("AZURE_EMBEDDING_DEPLOYMENT", AZURE_EMBEDDING_DEPLOYMENT)
...
def generate_embeddings(texts: List[str]) -> List[List[float]]:
    try:
        ...
        print("Azure embedding request failed:", e)
        raise
...
async def upload_documents(files: List[UploadFile] = File(...)):
    ...
    try:
        ...
        print(f"Upload error: {e}")  # PRINT FULL ERROR TO TERMINAL
...
async def query_endpoint(query: QueryRequest):
    try:
        ...
        question_embedding = generate_embeddings([query.question])[0]

        # Query Pinecone for top matches
        results = query_vector_store(vector=question_embedding, namespace=query.session_id)

        # Extract context from metadata
        context = [match["metadata"]["text"] for match in results["matches"]]

        # Generate response using Azure OpenAI chat
